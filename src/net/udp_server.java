	...
	private void udp() {
		try {
			while(true) {
				...
				System.out.println("상담원이 접속 하였습니다.");
				...
				InetAddress ia2 = this.dp.getAddress(); //상대방의 ip랑 port 가져옴
				int port2 = this.dp.getPort(); //상대방 port
				
		
				
				
				this.isr = new InputStreamReader(System.in); //scanner 없이 사용
				this.br = new BufferedReader(this.isr); //클라이언트 보낼 메세지를 메모리에 등록
				this.msg = this.br.readLine();
				byte client_msg[] = this.msg.getBytes();
				//해당 datagrampacket client 정보(byte 변수, byte길이, client_ip, client_port(udp))
